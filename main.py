...
if auth_choice == "Register":
    ...
    if st.sidebar.button("Register"):
        payload = {"name": reg_username, "password": reg_password}  # 👈 match backend API
        try:
            res = requests.post(f"{API_BASE}/user/register", json=payload)
    ...
    if st.sidebar.button("Login"):
        payload = {"name": login_username, "password": login_password}  # 👈 match backend API
        try:
            res = requests.post(f"{API_BASE}/user/login", json=payload)
            if res.status_code == 200:
                data = res.json()
                st.session_state.logged_in = True
                st.session_state.username = data["username"]
                st.success(f"✅ Welcome {data['username']}! Login successful.")
            else:
